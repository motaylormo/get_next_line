...
/*   Updated: 2018/11/11 15:56:22 by mtaylor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

t_gnl	*ft_listnew(int fd, t_gnl *prev, t_gnl *next)
{
	t_gnl *i;
...
t_gnl	*listfetch(t_gnl *list, int fd)
{
	if (!(list))
...
char	*get_overflow(char *old, char buf[])
{
	int		n;
	char	*new;
...
char	*update_str(char *str, char *buf)
{
	char	*temp;
	char	*buf_seg;
